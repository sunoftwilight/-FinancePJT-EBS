  ...
  <div class="container mx-2 mt-3">
    ...
    <div class="mx-2 mt-3">
      <div class="d-flex justify-content-between">
        ...
        <RouterLink :to="{ name: 'savingCompare'}" class="btn mybtn pt-1 me-3" style="height: 35px;">목록</RouterLink>
      </div>
      <hr>
      ...
      <div class="ms-2">
        <p class="my-4 text-primary fw-bold" v-if="test == 'F'">자유적립식</p>
        <p class="my-4 text-primary fw-bold" v-else>정액적립식</p>
        <div v-for="(option, index) in product.savingoptions_set" :key="index">
          <p v-if="test === option.rsrv_type"><span class="ms-1 text-primary opacity-75">{{ option.save_trm }}개월</span> &nbsp; {{ option.intr_rate }}</p>
        </div>
      </div>
      <hr>
...
const test = route.params.rsrv_type

const product = store.finances.savings.find((product) => product.id == userId)
